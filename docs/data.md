...
- `LAYERED_WORLD_MAP_CONF` — 分层地图(洞穴/地下层)切片图与投影(见 3.2 节)
...
进入洞穴/地下层时,大地图显示切换为该层的**局部切片图**(`LayerMap/` 单张,不是 4×4 瓦片),
投影换成该层的 `camera_center` + `Ortho_width`(而非底图的 `map_center`/`side_length`)。**坐标系
不变**(仍是所属 `scene_res_id`,如 10003),只换图和投影参数。数据源 `LAYERED_WORLD_MAP_CONF`
(15 行:每组 sort=1 是地表无图=用底图,sort=2+ 是各楼层带图)→ `layers` 索引(只收 9 个有图层)。

**层结构与楼层**:同一洞穴的一层/二层是**两张图**(`_01`/`_02`),`camera_center` 相同,但**可行走
区域在不同 X/Y**(实测信仰者村落二层在一层北侧,Y 更大),非同坐标上下叠。

**选层机制(客户端)**:位置(x,y)对 `AREA_CONF` 多边形做点在区域内判定(`GetPointAreaId`,原生)→
`area_func_id` → 层。这条不进协议,且 `AREA_CONF` **8.4MB/35592 行**(未 vendored)。

**协议侧粗定位**:`ZoneSceneClientCaveStateReq`(0x1838,c2s)/ `...Notify`(0x1505,s2c)下发
`cave_name`(string)+ `pos`。`cave_name`(如 `Cave_A2_02_01`)是层 `map_resource` 去掉
`_CaveTunnel...` 后的**前缀**(存入 `layers[].cave`),可定到**洞穴组**;组内楼层仍需多边形判定。
`DB.LayersByCave(cave_name)` 返回候选层。**已用真实洞穴 pcap 验证**:0x1838 解出
`cave_name="Cave_A2_02_01"`,魔力之源坐标经层投影正确落在信仰者村落切片图中央通路。

**切片图**(`gen_bigmap.py`):`LayerMap/<map_resource>.png` 单张转码 → `img/bigmap/layer/<map_resource>.webp`
(保持原名,不拼接);9 张合计约 300KB。下水管道口/家园层(地下室/二楼)无 `cave` 前缀(进层检测
机制不同,暂缓);家园层与底图 `30001_<房屋等级>` 有重叠。

**支持分档**:v1 忽略层(进洞穴时人物点仍显示在大陆底图对应坐标,位置正确只是无放大内嵌图);
v2 按 `cave_name` 前缀显示洞穴组切片图;v3 加候选楼层多边形判定做楼层精确(需 `AREA_CONF` 子集)。
当前已备齐 v2 数据(层表 + 切片图 + `layers` 索引 + `DB.LayersByCave`),v1/v2 前端与洞穴解析待做。
