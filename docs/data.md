...
行 id 同样跨版本稳定(实测大版本更新后星星刷新行、区域多边形行 id 与顶点原样不动)。

> **2026-07 大版本起,策划字段从发布数据剥离**:`editor_name`(AREA_CONF/NPC_REFRESH/SCENE_*/
> NPC_CONF/MODEL_CONF)、`max_num`/`storage_num`/`npc_pendant_id` 等不再随包下发。原依赖
> editor_name 的两处识别:石像奖励行排除改用**刷新区域多顶点**几何判别(与旧标注完全重合,
> 免维护,见 3.3);星星区域多边形改用 `scripts/legacy_editor_names.json` **快照回退**——该文件
> 取自最后一个带这些字段的版本(git f56bd6c 的 nrc/bin),按行 id 引用现行表。editor_name 若在
> 某版回归则仍优先生效;新版本若新增**有星区域**,多边形快照无法自动识别,需人工补充(见 3.3)。
...
- 独立星里**刷新区域是多顶点**的行:石像关联的**奖励星预设落点**(蓝 94 行:51 单星 + 43 多星,
  区域 2/6/12 顶点,旧版 `editor_name` 标注「眠枭石像N」、Σ`max_num`=277 颗)。实测石像的星走
  实体挂件、触碰即收(见 3.4),这些行未见刷出,不是常驻点位。**真星点的刷新区域全部单顶点**
  (三色全量验证),几何判别与被剥离的 editor_name 标注完全重合且免维护(紫独立星无奖励行);
  **不能**用「距最近石像的距离」替代(奖励行最远离石像 394.6m,而真独立星最近仅 3.6m);
- **装饰石像**:58303-58305(A1)/58313-58316(A2)/55633、55635、55636(新版-A2 名)共 248 行,
  与带星石像**坐标互不重合**,行 id 不在挂件表里(或旧数据里 `npc_pendant_id` 为空/共用 801385)
  ——石像上没挂自己的星,服务器也不计数。这些 NPC id 不入 `STAR_NPCS`;
- 50206「增加血上限_眠枭之星」(特殊星,大地图 6 行,另有 1 行 refresh_type=8 的任务刷星)与
  50240(旧版 `editor_name`「准备废弃的数据」,1 行 2 星):**它们同时是蓝/黄石像的挂件星 npc**,
...
范围内,底图重新生成即可):魔力之源 43(另魔法学院 10018 有 4)、炼金釜 28(另家园种植园 30002 有 1)、
守护地 33、大型眠枭庇护所 37、小型 11、蓝色眠枭之星 145(+2 见上)、黄色 228(蓝/黄刷新行与旧版
完全一致)、紫色 104(全在新区坐标带;另有 2 独立星在副本、3 光点行 + 2 石像行 disable)。
紫星所在新区无区域多边形(见上),z 全为 0 一律显示——仅 2 颗落在相邻旧区(甜水农庄/恶水沼泽)
多边形内;服务器对紫星的 belong_camp 归属待新区 pcap 验证。
...
**AOI 是按格子下发的,不是圆形半径**:`AREA_CONF` 里有一批 editor_name 为「跨aoi拆分」的区域
(带格子号如 `11032278_28*29`)。实测也反复出现「更远的实体下发了、更近的没下发」(154m 未发但
170m 发了;127m 未发但 146m 发了)。故不能拿单一半径当 AOI 边界,只能取一个**保守判定半径**:
4 份 pcap 里凡距玩家轨迹 **≤100m 的固定 POI(必定存在的那些)全部下发,无一例外**,故代码取 **80m**
(`starSweepRadius`)留足余量。进场景后还要等快照到齐(`starSettle`)再判,否则会把「还没下发」当成
「已收集」。

**按区域整片隐藏**:游戏内的星星**按区域计数**(商店街周边/月牙湖岸/风眠圣所…),进场景包里给出
每区域的「已收集/总数」:
...
六个 id);后端聚合同区域各形态为一条进度。某区域 `got==tot` ⇒ 该区域的星星全已收集,可整片隐藏,
不必逐点走到。

**星点→区域的归属**只能靠几何:区域行的 `name_area_id` 只是大地图上的**地名锚点(单顶点)**,不是范围;
真正的范围是 `AREA_CONF` 里一批**以区域名命名的多边形**(`editor_name[0] == zone_name`,旧 35 区中
34 个有,缺「圣羽祭台」)。点在多边形内即属该区域(names.json 的 POI `z`);落不进任何多边形的点
`z=0`,一律显示。`editor_name` 剥离后,这 126 行多边形按 id 固化在 `scripts/legacy_editor_names.json`
的 `zone_polygon_areas`(顶点仍从现行 AREA_CONF 读);2026-07 新增的 8 区没识别出多边形,但新区
也没有星星,不影响归属。
> 试过但**不成立**的路子(勿重蹈):`NPC_REFRESH_CONTENT_CONF.belong_camp`(星星那几百行全为空,
> 该字段只有野生宠物在用)、`CAMP_CONTENT_NPC_CONF`(空表)、「最近营地」归属(65 组只对上 11 组)、
> `editor_name` 里的区域名(多数行不带)。
...
28→27,该区实测 `got=2/tot=1`——已收集的记录保留,故 got 可能大于 tot);蓝独立星 98 里也有 4 颗
不计区域(计入 94)。故**不要拿配置点数当分母**算进度,进度一律用服务器给的 `explore_num/total_num`,且
`got>=tot` 才算收满。这不影响隐藏的正确性:区域没收满时全显示,收满时该区域计数内的点确实都已收集。
被移除的那颗光点实体永不下发,逐点判定会把它标成「已收集」——对没收过的人它本也不可再得,可接受。
