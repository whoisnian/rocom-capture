...
- `WORLD_MAP_CONF` + `NPC_REFRESH_CONTENT_CONF` + `AREA_CONF` + `SCENE_OBJECT_CONF` + `NPC_CONF`
  — 大地图 POI(炼金釜/魔力之源/…)的图标与坐标(见 3.3 节)。这几张是 `nrc/bin` 里最大的
  (AREA_CONF 8.1M、NPC_REFRESH 3.3M、NPC_CONF 2.4M),但坐标只能从它们来,故一并 vendored
...
| `worldmap` | 脚本内 `WORLDMAP` 清单 | 人工挑选的大地图 POI(炼金釜/魔力之源/守护地、矿石与植物标记、眠枭庇护所与眠枭之星、精灵果实) | 11 |
...
蓝/黄眠枭之星默认关,尤其后两者各有两百多个点)。图标本身早已由 `gen_icons.py` 的 worldmap 组切出
...
- `SCENE_OBJECT_AWARD` 与 `SCENE_OBJECT_CONF` **id 相同、含义不同**(前者是可采集物,如同 id 下是棵树),
  取错表会得到完全另一处的坐标。
- **眠枭之星是反向绑定**:它那 4 行(id 30000-30003)没有 `npc_refresh_ids`,而是
  `NPC_CONF.min_map_disappear` **存着 WORLD_MAP_CONF.id**。字段名看着像「小地图消失距离」,实测是外键:
  全库仅 9 行有此列,值 100% 是合法的 WORLD_MAP_CONF.id 且全为眠枭之星系 NPC。故要从 NPC 反查其刷新行。
  由此定出 **A1=蓝、A2=黄**(与 `gen_icons.py` 里 `owl_worldmap_fruit_A1`=蓝色精灵果实一致)。
- **只收游戏真会显示的元素**:`WORLD_MAP_CONF` 有 9 个显示开关(大地图/小地图/罗盘 × 未探索/已探索/
  未完成),全空 = 纯触发体。魔力之源就有 5 行「空npc,用于分层地图切换」(id 54001-54005,散落在真实
  魔力之源 65-260m 外),不滤掉会在图上多出 5 个假图标。**不能只看大地图那 3 个开关**——眠枭之星按设计
  只上小地图/罗盘(大地图开关全空),那样会被全部误杀。刷新行的 `disable` 同样要跳过。
...
尺寸不随缩放变大)。只收**有底图的场景**:副本/独立洞穴场景(20xxx)无从投影,那里的眠枭之星(黄 41、
蓝 23 个)不入库;洞穴/楼层的点仍属地表 res(如 10003),照常落在底图上。
...
**点数**(启用行,卡洛西亚大陆 10003):魔力之源 35(另魔法学院 10018 有 4)、炼金釜 24(另家园种植园
30002 有 1)、守护地 26、大型眠枭庇护所 29、小型 10、蓝色眠枭之星 224、黄色 194。

## 3.4 眠枭之星的收集状态(实时地图页「收集模式」)

**核心事实(pcap 实测)**:**已收集的星星服务器根本不刷**。未收集的才作为 NPC 实体(`ActorInfo`)
...
`belong_camp` 的键是该区域**营地(魔力之源)的刷新点 id**;`WORLD_MAP_CONF` 的 1-35 号行是区域行
(`zone_name` + `camp_refresh_id`),据此翻成中文区域名(names.json 的 `zones`)。某区域 `got==tot`
⇒ 该区域的星星全已收集,可整片隐藏,不必逐点走到。

**星点→区域的归属**只能靠几何:区域行的 `name_area_id` 只是大地图上的**地名锚点(单顶点)**,不是范围;
真正的范围是 `AREA_CONF` 里一批**以区域名命名的多边形**(`editor_name[0] == zone_name`,34/35 个区域有,
缺「圣羽祭台」)。点在多边形内即属该区域(names.json 的 POI `z`);落不进任何多边形的点 `z=0`,一律显示。
> 试过但**不成立**的路子(勿重蹈):`NPC_REFRESH_CONTENT_CONF.belong_camp`(星星那 474 行全为空,
> 该字段只有野生宠物在用)、`CAMP_CONTENT_NPC_CONF`(空表)、「最近营地」归属(65 组只对上 11 组)、
> `editor_name` 里的区域名(474 行只有 89 行带)。

**注意口径**:配置里的候选星点(474)比服务器统计的总数(312)多——有些点位实际不刷,且服务器只统计
已解锁区域。故**不要拿配置点数当分母**算进度,进度一律用服务器给的 `explore_num/total_num`。这不影响
隐藏的正确性:区域没收满时全显示,收满时该区域的点确实都已收集(连带隐藏多余候选点无妨)。
