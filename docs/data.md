...
**选层机制:位置点在多边形内判定(唯一完整机制)**。游戏按玩家位置对该层的区域多边形做点在区域内
判定(客户端 `GetLayerIdByPos`)→ 层。传送/移动进入、开阔洞穴区(月兔暗港)、楼层区分(一层/二层
多边形在不同 X/Y)**皆据此**。协议侧的 `cave_name`(`ZoneSceneClientCaveStateReq` 0x1838)**只在传送
进流送洞穴时才发,移动进入不发**(实测飞入月兔暗港无 0x1838),故**不可靠,已弃用**。

多边形来自 `AREA_CONF`:层 `area_func_id` → `AREA_FUNC_CONF.area_id` → `AREA_CONF.pos`(世界坐标顶点,
取顶点≥3 的多边形)。写进 `layers[].poly`(39 个多边形共约 8KB)。`AREA_CONF` **8.4MB/35592 行未
vendored**,`gen_gamedata.py` 仅在 `NRC_AREA_DIR`(默认 Downloads)存在时读取提取,不存在时**保留上一版
names.json 已烘焙的 poly**(故常规重跑无需 Downloads;更新地图时才需)。`DB.LayerAt(res,x,y)` 遍历该
场景层的多边形做射线法判定,命中即该层。层的 `res`:优先 LAYERED 表 `scene_res_id`,家园层(表内为空)
从其区域行的 `scene_res_id` 补齐(=30001),否则 `LayerAt(30001,..)` 会漏掉家园楼层。

**叠加渲染**:层世界范围 `[OX,OX+Side]×[OY,OY+Side]` 投影为底图归一化矩形(u0,v0)-(u1,v1)放进 payload
的 `layer{img,u0,v0,u1,v1}`,前端把切片图(`.map-layer`)定位在 `.map-world` 内对应位置(透明处透出
底图);玩家点仍用底图投影,自然落在矩形内。**进/出层不改缩放**(与外层保持一致):缩放/跟随只在底图
(`pos.img`,即换场景/家园等级)变化时重置,层图变化只重试层图。

**切片图**(`gen_bigmap.py`):`LayerMap/<map_resource>.png` 单张转 `img/bigmap/layer/<map_resource>.webp`
(保持原名、保留 alpha);9 张约 860KB。

**已用真实 pcap 验证(洞穴 + 家园楼层)**:轨迹地表→飞入月兔暗港→传送信仰者村落一层,及家园
一楼(地表)→跑上二楼(z≈312)→经一楼→下地下室(z≈-598),`LayerAt` 逐点判定层变化均正确
(移动进入触发、地表不误判、楼层区分对),切片叠在对应位置、箭头落点对、缩放不变。家园楼层切片
(`camera_center`/`Ortho_width` 与家园底图一致)故矩形≈全图,楼层平面图覆盖整张家园底图(全屏视图)。
